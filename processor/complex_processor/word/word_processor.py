...
class WordProcessor(BaseComplexProcessor):
    """Converts a Word document to PDF and processes it as a PDF.
    ...
    Config format (``word_processor_config.yaml``)::
    ...
    @classmethod
    def build(cls, config_path: str | Path) -> "WordProcessor":
        import yaml
    ...
    def run(self, source: str | Path | bytes) -> Word:
        ...
        return Word(photos=pdf.photos, file_name=file_name)
    ...
    def _convert_to_pdf(self, source: str | Path | bytes) -> bytes:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if isinstance(source, bytes):
                src_path = tmp_dir / "input.docx"
                src_path.write_bytes(source)
            else:
                src_path = Path(source)
                if not src_path.exists():
                    raise FileNotFoundError(f"word document not found: {src_path}")
