...
class ODTProcessor(BaseComplexProcessor):
    """Converts an OpenDocument Text (.odt) document to PDF and processes it as a PDF.
    ...
    Config format (``odt_processor_config.yaml``)::
    ...
    @classmethod
    def build(cls, config_path: str | Path) -> "ODTProcessor":
        import yaml
    ...
    def run(self, source: str | Path | bytes) -> ODT:
        ...
        return ODT(photos=pdf.photos, file_name=file_name)
    ...
    def _convert_to_pdf(self, source: str | Path | bytes) -> bytes:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if isinstance(source, bytes):
                src_path = tmp_dir / "input.odt"
                src_path.write_bytes(source)
            else:
                src_path = Path(source)
                if not src_path.exists():
                    raise FileNotFoundError(f"odt document not found: {src_path}")
            ...
            if not pdfs:
                raise ConfigurationError(
                    "soffice produced no PDF output for the document",
                )
