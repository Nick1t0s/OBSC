...
class PowerPointProcessor(BaseComplexProcessor):
    """Converts a PowerPoint presentation to PDF and processes it as a PDF.
    ...
    Config format (``powerpoint_processor_config.yaml``)::
    ...
    @classmethod
    def build(cls, config_path: str | Path) -> "PowerPointProcessor":
        import yaml
    ...
    def _convert_to_pdf(self, source: str | Path | bytes) -> bytes:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if isinstance(source, bytes):
                src_path = tmp_dir / "input.pptx"
                src_path.write_bytes(source)
            else:
                src_path = Path(source)
                if not src_path.exists():
                    raise FileNotFoundError(f"powerpoint not found: {src_path}")
