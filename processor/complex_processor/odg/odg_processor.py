...
class ODGProcessor(BaseComplexProcessor):
    """Converts an OpenDocument Drawing (.odg) to PDF and processes it as a PDF.
    ...
    Config format (``odg_processor_config.yaml``)::
    ...
    @classmethod
    def build(cls, config_path: str | Path) -> "ODGProcessor":
        import yaml
    ...
    def run(self, source: str | Path | bytes) -> ODG:
        ...
        return ODG(photos=pdf.photos, file_name=file_name)
    ...
    def _convert_to_pdf(self, source: str | Path | bytes) -> bytes:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if isinstance(source, bytes):
                src_path = tmp_dir / "input.odg"
                src_path.write_bytes(source)
            else:
                src_path = Path(source)
                if not src_path.exists():
                    raise FileNotFoundError(f"odg document not found: {src_path}")
            ...
            if not pdfs:
                raise ConfigurationError(
                    "soffice produced no PDF output for the drawing",
                )
